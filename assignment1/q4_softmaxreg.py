...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    # sentVector = np.sum(wordVectors[indices, :], axis=0) / len(indices)
    ### END YOUR CODE
    
    return sentVector

def softmaxRegression(features, labels, weights, regularization = 0,
                      nopredictions = False):
    """ Softmax Regression """
    ...
    #计算prob矩阵中，每行Xi对应Yi的label处的概率的熵，
    #  Loss_CE = sigma(p(j)*-log(p(i))),此处P(j)为1.．
    cost = np.sum(-np.log(prob[range(N), labels])) / N 
    cost += 0.5 * regularization * np.sum(weights ** 2)

    ### YOUR CODE HERE: compute the gradients and predictions
    grad = np.array(prob)
    #殘差delta = (softmax(x) - labels) / N
    grad[range(N), labels] -= 1.0
    grad /= N
    #参考q2_neural
    grad = np.dot(features.T, grad)
    #Ｌ２正则对W的导数．
...
def sanity_check():
    ...
    for i in range(10):
        ...
        print('words:{}, labels:{}'.format(words, dummy_labels[i]))
