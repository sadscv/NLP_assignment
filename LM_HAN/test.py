...
class RNNLM_Model(LanguageModel):
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'), tf.variable_scope("embedding"):
            ### YOUR CODE HERE
            self.L = tf.get_variable("L", shape=[len(self.vocab), self.config.embed_size])
            ...
            inputs = tf.split(inputs, num_or_size_splits=self.config.num_steps, axis=0)
    ...
    def add_projection(self, rnn_outputs):
        ...
        with tf.variable_scope("projection"):
            ...
            outputs = [tf.nn.xw_plus_b(x=output, weights=U, biases=b_2) for output in rnn_outputs]
        ### END YOUR CODE
        return outputs
    ...
    def add_loss_op(self, output):
        ...
        ce = sequence_loss(logits=[output], targets=[tf.reshape(tensor=self.labels_placeholder, shape=[-1])],
                           weights=all_ones)
        # 得到所有的误差.如果要考虑L2正则化,可以考虑将I和H以及U加入total_loss
    ...
    def __init__(self, config):
        ...
        output = tf.reshape(tf.concat(values=self.outputs, axis=1), [-1, len(self.vocab)])
    ...
    def add_model(self, inputs):
        ...
        with tf.variable_scope("input_dropout"):
            inputs = [tf.nn.dropout(input, keep_prob=self.dropout_placeholder) for input in inputs]
        ...
        with tf.variable_scope("output_dropout"):
            rnn_outputs = [tf.nn.dropout(output, keep_prob=self.dropout_placeholder) for output in rnn_outputs]
        ### END YOUR CODE
        return rnn_outputs
    ...
    with tf.Session() as session:
        ...
        for epoch in xrange(config.max_epochs):
            ...
            if valid_pp < best_val_pp:
                ...
                saver.save(session, 'data/ptb/ptb_rnnlm.weights')
            if epoch - best_val_epoch > config.early_stopping:
                break
            print 'Total time: {}'.format(time.time() - start)

        saver.restore(session, 'data/ptb/ptb_rnnlm.weights')
