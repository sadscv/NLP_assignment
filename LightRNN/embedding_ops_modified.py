...
def embedding_lookup(params, ids, partition_strategy="mod", name=None,
  ...
  with ops.name_scope(name, "embedding_lookup", params + [ids]) as name:
    ...
    #如果params不止一个的话。
    # else:
    #     #将ids转化为Tensor.不管它是list、numpy.arrary还是标量。
    ...
    #   partitioned_result = []
    #     #对于每个params.使用gather_id来获取param中与ID对应的向量。
    #   for p in xrange(np):
    #     with ops.colocate_with(params[p]):
    ...
    #                             # 维度拼接，相当于是ids的维度+每个embedding的维度。
    #                             array_ops.concat(
